...
UI_SITES = {
  ...
  class Global < Connector
    ...
    #
    ###################################################################################
    def get_button(buid, add_params = nil)
      params = {
        buid: buid
      }
  ...
  class ConversationProxy < Connector
    ...
    ##################################################################################
    # Creates a record from a token, either a token that was generated by the client_id
    ...
    #
    ###################################################################################
    def delete_affinity(user_affinity_token, add_params = nil)
      params = {
      uid: uid,
      user_affinity_token: user_affinity_token,
      }
      api_call('/conversation_proxies/:uid/affinities/:user_affinity_token(.:format)',:delete,params,add_params)
    end
    ...
    #
    ###################################################################################
    def expose_affinity(user_affinity_token, add_params = nil)
      params = {
      uid: uid,
      user_affinity_token: user_affinity_token,
      }
      api_call('/conversation_proxies/:uid/affinities/:user_affinity_token/expose(.:format)',:post,params,add_params)
    end
    ...
    def get_affinity_session(user_affinity_token, add_params = nil)
      ...
      api_call('/conversation_proxies/:uid/affinities/:user_affinity_token/session(.:format)',:get,params,add_params)
    end
  ...
  class Store < Connector
    ...
    def self.create_by_token(token,site_type,add_params)
      store = new(token,site_type)
      record = store.send(:create,add_params)
      unless record.key?(:store) && record[:store].key?(:uid)
        if record[:error]
           raise record[:error]
        else
      ...
      store.store_rec = record
      store.uid = record[:store][:uid]
      store
    end
    ...
    #
    ###################################################################################
    def upload_product_image(rid, add_params = nil)
    ...
    #
    ###################################################################################
    def get_button_can(buid, nuid, add_params = nil)
      params = {
      uid: uid,
      buid: buid,
      nuid: nuid,
    ...
    alias_method :get_store_button_can, :get_button_can
    ...
    #
    ###################################################################################
    def close_button_can(buid, nuid, add_params = nil)
      params = {
      uid: uid,
      buid: buid,
      nuid: nuid,
    ...
    alias_method :close_store_button_can, :close_button_can
  ...
  class PartnerProxy < Connector
    ...
    def self.authenticate(uid,client_id,client_secret,site_type,add_params=nil)
      ...
      unless record.key?(:partner)
        if record[:error]
          ...
          raise 'Unable to authenticate Partner proxy!'
        end
      end
    ...
    def delete_affinity(user_affinity_token, add_params = nil)
      ...
      api_call('/partner_proxies/:uid/affinities/:user_affinity_token(.:format)',:delete,params,add_params)
    end
    ...
    def expose_affinity(user_affinity_token, add_params = nil)
      ...
      api_call('/partner_proxies/:uid/affinities/:user_affinity_token/expose(.:format)',:post,params,add_params)
    end
